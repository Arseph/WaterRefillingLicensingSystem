...
class AlertNotification extends Controller
{
    ...
    {
        $facilities = FacilityInspection::with('facility')->whereRaw('LOWER(`inspection_status`) LIKE ? ', [trim(strtolower('Pending Inspection')) . '%'])->orWhereRaw('LOWER(`inspection_status`) LIKE ? ', [trim(strtolower('Pending Reinspection')) . '%'])->get();
        ...
        if (count($facilities) > 0) {
            foreach ($facilities as $facinspect) {
                if (
                    ...
                    $facinspect->inspection_date != "" && $facinspect->inspection_status != "Finished"
                ) {
                    $client = UserProfile::find($facinspect->facility->user_id);
                    ...
                    if (Carbon::parse($formattedDate) <= $currentDate) {
                        ...
                        if ($overdueDays > 0) {
                            ...
                            array_push($overdues, [
                                ...
                                'client'=>$client->firstname . ' ' . $client->mi[0] . '. ' . $client->lastname,
